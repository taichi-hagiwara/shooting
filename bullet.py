...
from constants import GRAY, CELL_SIZE, GRID_SIZE, MAX_BULLETS, BULLET_RECHARGE_TIME

class Bullet:
  # 残弾数の初期化をクラス変数で管理
  max_bullets = MAX_BULLETS  # 最大装填数
  current_bullets = max_bullets  # 現在の残弾数
  bullet_timer = 0  # 弾補充用タイマー
  ...
  @classmethod
  def recharge_bullet(cls):
    cls.bullet_timer += 1
    if cls.bullet_timer >= BULLET_RECHARGE_TIME and cls.current_bullets < cls.max_bullets:
      cls.bullet_timer = 0  # タイマーをリセット
      cls.current_bullets += 1  # 弾を補充
  ...
  def update(self, grid):
    ...
    # マス座標を計算 (グリッドの位置に変換)
    grid_pos = Vector2(int(self.pos.x // CELL_SIZE),
